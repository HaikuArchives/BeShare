...
public:
	...
	status_t StartProcessing(int32 replyCode, const MessageRef & msgRef, const BMessenger & replyTo, volatile bool shutdownFlag)
	{
		...
		if(_shutdownFlagMD5)
			TRACE_SHAREFILETRANFER(("MD5Looper::StartProcessing _shutdownFlagMD5 = true\n"));
		else 
			TRACE_SHAREFILETRANFER(("MD5Looper::StartProcessing _shutdownFlagMD5 = false\n"));
			
		_shutdownFlagMD5 = shutdownFlag;
		
		if(shutdownFlag)
			TRACE_SHAREFILETRANFER(("MD5Looper::StartProcessing shutdownFlag = true\n"));
		else 
			TRACE_SHAREFILETRANFER(("MD5Looper::StartProcessing shutdownFlag = false\n"));
...
public:
	...
	virtual void MessageReceived(BMessage*)
	{
		...
		Message & msg = *_msgRef();
		TRACE_SHAREFILETRANFER(("MD5SendLooper::MessageReceived Files nr of files %d\n", _fileSet.GetNumItems()));
		for (HashtableIterator<String, OffsetAndPath> iter(_fileSet.GetIterator()); iter.HasData(); iter++) {
			if (_shutdownFlagMD5 != NULL)
				TRACE_SHAREFILETRANFER(("MD5SendLooper::MessageReceived _shutdownFlagMD5 != NULL\n"));
				
			if (_shutdownFlagMD5 != false)
				TRACE_SHAREFILETRANFER(("MD5SendLooper::MessageReceived ((*_shutdownFlagMD5) != false\n"));
			
			if ((_shutdownFlagMD5 != NULL) && (_shutdownFlagMD5 != false)){
				TRACE_SHAREFILETRANFER(("MD5SendLooper::MessageReceived _shutdownFlagMD5 != NULL) || (_shutdownFlagMD5 != false\n"));
				break;
			}

			TRACE_SHAREFILETRANFER(("MD5SendLooper::MessageReceived Files %s\n", iter.GetKey().Cstr()));
...
public:
	virtual void MessageReceived(BMessage *)
	{
		...
		Reply();
		TRACE_SHAREFILETRANFER(("MD5ReceiveLooper::MessageReceived end\n"));
	}
};
...
{
	TRACE_SHAREFILETRANFER(("ShareFileTransfer::SetFromArchive begin\n"));
	_origFileSet.Clear();
	_errorOccurred = true;
	...
	TRACE_SHAREFILETRANFER(("ShareFileTransfer::SetFromArchive end\n"));
}
...
{
	TRACE_SHAREFILETRANFER(("ShareFileTransfer::InitConnectSession begin\n"));
	_banEndTime = 0;
	_isAcceptSession = false;
	...
	TRACE_SHAREFILETRANFER(("ShareFileTransfer::InitConnectSession end\n"));
	return B_NO_ERROR;
}
...
{
	TRACE_SHAREFILETRANFER(("ShareFileTransfer::InitAcceptSession begin\n"));
	_banEndTime = 0;
	...
	TRACE_SHAREFILETRANFER(("ShareFileTransfer::InitAcceptSession end\n"));
	return ret;
}
...
ShareFileTransfer::MessageReceived(BMessage* msg)
{
	switch(msg->what)
	{
		...
		{
			TRACE_SHAREFILETRANFER(("ShareFileTransfer::MessageReceived : BMessage : BMessage (TRANSFER_COMMAND_MD5_RECV_READ_DONE)\n"));
				...
				while((_mtt)&&(_mtt->GetNextEventFromInternalThread(code, &next) >= 0))
				{
					switch(code)
					{
						case MTT_EVENT_SESSION_ACCEPTED:
							TRACE_SHAREFILETRANFER(("ShareFileTransfer::MessageReceived : BMessage (MTT_EVENT_SESSION_ACCEPTED)\n"));
							_mtt->RemoveAcceptFactory(0);	// now that we have our connection, no need for the factory!
							// fall thru!
						case MTT_EVENT_SESSION_CONNECTED:
						{
							TRACE_SHAREFILETRANFER(("ShareFileTransfer::MessageReceived : BMessage (MTT_EVENT_SESSION_CONNECTED)\n"));
							_isConnecting = false;
							_isConnected = _wasConnected = true;
							if (_mtt)	// paranoia
							{
								TRACE_SHAREFILETRANFER(("ShareFileTransfer::MessageReceived : BMessage (MTT_EVENT_SESSION_CONNECTED) _mtt\n"));
								if (_uploadSession)
								{
								...
								{
									TRACE_SHAREFILETRANFER(("ShareFileTransfer::MessageReceived : BMessage (MTT_EVENT_SESSION_CONNECTED) build\n"));
									...
									TRACE_SHAREFILETRANFER(("ShareFileTransfer::MessageReceived : BMessage (MTT_EVENT_SESSION_CONNECTED) begin fileRequest\n"));
									if (fileRequest())
									{
										TRACE_SHAREFILETRANFER(("ShareFileTransfer::MessageReceived : BMessage (MTT_EVENT_SESSION_CONNECTED) has fileRequest\n"));
										TRACE_SHAREFILETRANFER(("ShareFileTransfer::MessageReceived : BMessage (MTT_EVENT_SESSION_CONNECTED) nr of files %d\n", _fileSet.GetNumItems()));
										...
										if(_shutdownFlag)
											TRACE_SHAREFILETRANFER(("ShareFileTransfer::MessageReceived : BMessage (MTT_EVENT_SESSION_CONNECTED) _shutdownFlag = true\n"));
										else if(!_shutdownFlag)
											TRACE_SHAREFILETRANFER(("ShareFileTransfer::MessageReceived  _shutdownFlag = false\n"));
										if (md5Looper->StartProcessing(TRANSFER_COMMAND_MD5_SEND_READ_DONE, fileRequest, BMessenger(this), _shutdownFlag) == B_NO_ERROR) 
										{
											TRACE_SHAREFILETRANFER(("ShareFileTransfer::MessageReceived : BMessage (MTT_EVENT_SESSION_CONNECTED) StartProcessing running\n"));
											_md5Loopers.Put(md5Looper, true);
											ResetTransferTime();	// don't kill us due to inactivity while we're checksumming!
											((ShareWindow*)Looper())->RefreshTransferItem(this);
										} else {
											TRACE_SHAREFILETRANFER(("ShareFileTransfer::MessageReceived : BMessage (MTT_EVENT_SESSION_CONNECTED) AbortSession 1\n"));
											delete md5Looper;	// ???
											AbortSession(true);
										}
									} else {
										TRACE_SHAREFILETRANFER(("ShareFileTransfer::MessageReceived : BMessage (MTT_EVENT_SESSION_CONNECTED) AbortSession 2\n"));
										AbortSession(true);
									}
								}
							} else
								TRACE_SHAREFILETRANFER(("ShareFileTransfer::MessageReceived : BMessage (MTT_EVENT_SESSION_CONNECTED) no _mtt!!!\n"));
						...
						{
							//msg->PrintToStream();
							TRACE_SHAREFILETRANFER(("ShareFileTransfer::MessageReceived : BMessage (MTT_EVENT_SESSION_DISCONNECTED)\n"));
							...
							TRACE_SHAREFILETRANFER(("MD5SendLooper::MessageReceived setting _shutdownFlag = true (MTT_EVENT_SESSION_DISCONNECTED)\n"));
							_isFinished = true;
							_isConnecting = _isConnected = _isAccepting = _isWaitingOnLocal = _isWaitingOnRemote = false;
							if ((_currentFileBytesDone < _currentFileSize)||(_currentFileIndex < _origFileSetSize)) {
								_errorOccurred = true;
							} else {
						...
						case MTT_EVENT_INCOMING_MESSAGE:
							TRACE_SHAREFILETRANFER(("ShareFileTransfer::MessageReceived : BMessage (MTT_EVENT_INCOMING_MESSAGE)\n"));
							MessageReceived(next);
						break;

						case MTT_EVENT_OUTPUT_QUEUES_DRAINED:
							TRACE_SHAREFILETRANFER(("ShareFileTransfer::MessageReceived : BMessage (MTT_EVENT_OUTPUT_QUEUES_DRAINED)\n"));
							if (_uploadSession) {
						...
						case MTT_EVENT_SESSION_ATTACHED:
							TRACE_SHAREFILETRANFER(("ShareFileTransfer::MessageReceived : BMessage (MTT_EVENT_SESSION_ATTACHED)\n"));
							MessageReceived(next);
						break;
						
						case MTT_EVENT_SESSION_DETACHED:
							TRACE_SHAREFILETRANFER(("ShareFileTransfer::MessageReceived : BMessage (MTT_EVENT_SESSION_DETACHED)\n"));
							MessageReceived(next);
						break;
						
						case MTT_EVENT_FACTORY_ATTACHED:
							TRACE_SHAREFILETRANFER(("ShareFileTransfer::MessageReceived : BMessage (MTT_EVENT_FACTORY_ATTACHED)\n"));
							MessageReceived(next);
						break;
						
						case MTT_EVENT_SERVER_EXITED:
							TRACE_SHAREFILETRANFER(("ShareFileTransfer::MessageReceived : BMessage (MTT_EVENT_SERVER_EXITED)\n"));
							MessageReceived(next);
						break;
					}
				}
...
{
	TRACE_SHAREFILETRANFER(("ShareFileTransfer::DoUploadAux begin\n"));
	...
	if (_mtt)
	{
		if (_currentFileBytesDone >= 0)
		{
			...
			{
				TRACE_SHAREFILETRANFER(("ShareFileTransfer::DoUploadAux WARN_OUT_OF_MEMORY\n"));
				WARN_OUT_OF_MEMORY;
				AbortSession(true);
				return false;
			}
	...
	AbortSession(true);
	TRACE_SHAREFILETRANFER(("ShareFileTransfer::DoUploadAux end\n"));
	return false;
}
...
{
	TRACE_SHAREFILETRANFER(("ShareFileTransfer::MessageReceived : MessageRef begin \n"));
	...
	switch(msg->what)
	{
		...
		case TRANSFER_COMMAND_FILE_LIST:
		{
			...
			//const char * file1;
			if (_uploadSession)
				TRACE_SHAREFILETRANFER(("ShareFileTransfer::MessageReceived : MessageRef (TRANSFER_COMMAND_FILE_LIST) _uploadSession = true \n"));
			
			//msg->FindString("files", &file1);	// make sure he's asking for at least one file
			...
			if ((_uploadSession) && (msg->FindString("files", &file) == B_NO_ERROR)) {	// make sure he's asking for at least one file
				...
				TRACE_SHAREFILETRANFER(("ShareFileTransfer::MessageReceived : MessageRef (TRANSFER_COMMAND_FILE_LIST) AbortSession\n"));
				AbortSession(true);	// you're going the wrong way buddy!
			}
		}
		TRACE_SHAREFILETRANFER(("ShareFileTransfer::MessageReceived : MessageRef (TRANSFER_COMMAND_FILE_LIST) end \n"));
		break;		
		...
		{
			TRACE_SHAREFILETRANFER(("ShareFileTransfer::MessageReceived : MessageRef (TRANSFER_COMMAND_FILE_DATA) \n"));
			uint8 * data;
			size_t numBytes;
	...
	TRACE_SHAREFILETRANFER(("ShareFileTransfer::MessageReceived : MessageRef end \n"));
}
...
{
	TRACE_SHAREFILETRANFER(("ShareFileTransfer::SharesScanComplete begin\n"));
	...
	if (msg) {
		...
		for (int i=0; (msg->FindString("files", i, &nextFile) == B_NO_ERROR); i++) {
			TRACE_SHAREFILETRANFER(("ShareFileTransfer::SharesScanComplete file = %s\n", nextFile));
			String nextPath;
		...
		if(_shutdownFlag)
			TRACE_SHAREFILETRANFER(("ShareFileTransfer::SharesScanComplete _shutdownFlag = true\n"));
		else if(!_shutdownFlag)
			TRACE_SHAREFILETRANFER(("ShareFileTransfer::SharesScanComplete _shutdownFlag = false\n"));
		
		if (md5Looper->StartProcessing(TRANSFER_COMMAND_MD5_RECV_READ_DONE, temp, BMessenger(this), _shutdownFlag) == B_NO_ERROR) {
			TRACE_SHAREFILETRANFER(("ShareFileTransfer::SharesScanComplete StartProcessing running\n"));
			SendNotifyQueuedMessage();	// so they know they are waiting for us to get done...
			_md5Loopers.Put(md5Looper, true);
			ResetTransferTime();	// don't kill us due to inactivity while we're checksumming!
			((ShareWindow*)Looper())->RefreshTransferItem(this);
	...
	TRACE_SHAREFILETRANFER(("ShareFileTransfer::SharesScanComplete end\n"));
}
...
{
	TRACE_SHAREFILETRANFER(("ShareFileTransfer::AbortSession begin\n"));
	if ((error == false) || (forceNoRestart))
	...
	TRACE_SHAREFILETRANFER(("ShareFileTransfer::AbortSession end\n"));
}
...
{
	TRACE_SHAREFILETRANFER(("ShareFileTransfer::DrawItem start\n"));
	ShareWindow* win = (ShareWindow*) Looper();
	
	if ((win == NULL) || (itemRect.Width() <= 0) || (itemRect.Height() <= 0))
	...
	if (doubleBufferBitmap->Lock()) {
		...
		if (_errorOccurred) {
			if (_banEndTime > 0) {
					...
					if (_banEndTime == ((uint64)-1))
						...
						if (t) {
							...
							if (t) {
								...
								struct tm now = *t;
								if ((now.tm_year != then.tm_year) || (now.tm_yday != then.tm_yday)) {
				...
				TRACE_SHAREFILETRANFER(("ShareFileTransfer::DrawItem _banEndTime = %d\n", _banEndTime));
	...
	TRACE_SHAREFILETRANFER(("ShareFileTransfer::DrawItem end\n"));
}
...
{
	TRACE_SHAREFILETRANFER(("ShareFileTransfer::BeginTransfer begin\n"));
	...
	UpdateTransferTime();
	TRACE_SHAREFILETRANFER(("ShareFileTransfer::BeginTransfer end\n"));
}
...
{
	TRACE_SHAREFILETRANFER(("ShareFileTransfer::RestartSession begin\n"));
	...
	_isWaitingOnLocal = true;
	_currentFileIndex = 0;
	TRACE_SHAREFILETRANFER(("ShareFileTransfer::RestartSession setting _shutdownFlag = false\n"));
	...
	TRACE_SHAREFILETRANFER(("ShareFileTransfer::RestartSession end\n"));
}
...
{
	TRACE_SHAREFILETRANFER(("ShareFileTransfer::TransferCallbackRejected begin\n"));
	_banEndTime = (banTimeLeft >= BESHARE_UNKNOWN_BAN_TIME_LEFT) ? banTimeLeft : GetCurrentTime64()+banTimeLeft;
	AbortSession(true, true);
	TRACE_SHAREFILETRANFER(("ShareFileTransfer::TransferCallbackRejected end\n"));
}
